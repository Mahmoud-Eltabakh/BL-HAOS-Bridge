...
- Give the PipeWire player the buffer the PulseAudio player always had. `pw-play` was spawned without `--latency`, so it used its own 100 ms default while the PulseAudio path passed 250 ms: the transport this add-on actually uses held a fifth of the buffer, and audio that played for a moment and then broke up is exactly what a starved A2DP sink sounds like. Both clients now hold the same buffer, 500 ms by default (A bare `--latency` number means *samples*, so the flag carries its unit: without it the value would have been read as 250 samples.)
- Make the buffer an add-on option. `playback_buffer_ms` (50-2000 ms, default 500) is read by the daemon's s6 script and handed over as `BLHAOS_PLAYBACK_BUFFER_MS`, so a link that needs more slack than the default can be given more without a rebuild. It is deliberately bounded in both directions: more buffer is also more audio queued ahead of the speaker, which is how long audio keeps playing after a pause.
- Read the speaker's real volume back. A Bluetooth speaker keeps its own volume and AVRCP absolute volume moves the sink when the speaker's own buttons or app are used, and nothing in the audio graph told the bridge about it - so Home Assistant and the dashboard kept showing the last level written from Home Assistant while the speaker sat somewhere else. The sink is now read back once when the speaker connects and every 30 seconds after that (`wpctl get-volume` or `pactl get-sink-volume`, per transport) and the new level is published to both surfaces. A read-back within 1% is treated as no change, because AVRCP absolute volume is quantised to 7 bits and a slider must not jitter under the operator's finger.
- Apply a speaker's saved volume to the speaker. The per-speaker volume was documented as applied when a speaker registers, but only the reported number was set: the speaker kept the level it was already at. It is now applied when the speaker connects and again when the setting is saved, and read back afterwards so the published level is what the speaker actually took.
- Stop inventing a volume. `get_volume` fell back to the global 70% default for any speaker whose level had not been read, and the dashboard card turned a missing volume into 70% as well: both looked exactly like a real level. An unread speaker now reports its own configured volume, and the card shows no number (with the slider disabled) until the bridge has read one.
- Keep the level when a read-back fails. A speaker that went away or a sink that is not published must not blank the level the sliders are showing; the last known value stands and the failure is logged at `debug`.
- Name a muted sink in the log. A sink muted in the audio server still reports its level, so silence behind a non-zero slider is now explicable instead of mysterious.
